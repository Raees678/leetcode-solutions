...
class Solution {
 public:
  int findContentChildren(vector<int>& greed_factors, vector<int>& sizes) {
    std::sort(greed_factors.begin(), greed_factors.end());
    std::sort(sizes.begin(), sizes.end());
    int number_of_cookies = sizes.size();
    int number_of_children = greed_factors.size();

    int satisfied_children = 0;
    while (number_of_cookies && number_of_children) {
      if (greed_factors[number_of_children - 1] <=
          sizes[number_of_cookies - 1]) {
        ++satisfied_children;
        --number_of_cookies;
      }
      --number_of_children;
