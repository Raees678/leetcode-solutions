...
class Solution {
 public:
  Node* dfs_visit(Node* node, vector<Node*>& copied_nodes) {
    int val = node->val;
    if (!copied_nodes[val]) {
      Node* copy = new Node(val);
      copied_nodes[val] = copy;
      for (Node* neighbor : node->neighbors) {
        Node* copied_neighbor = dfs_visit(neighbor, copied_nodes);
        copy->neighbors.push_back(copied_neighbor);
      }
      return copy;
    }
    return copied_nodes[val];
  }
  ...
  Node* cloneGraph(Node* node) {
    ...
    vector<Node*> copied_nodes(101, nullptr);
    return dfs_visit(node, copied_nodes);
  }
};
// @lc code=end
